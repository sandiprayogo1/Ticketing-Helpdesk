...
const Index = ({ auth }: CustomPageProps) => {
  const { karyawan } = usePage<CustomPageProps>().props
            ...
            <Table>
              <TableHeader>
                <TableRow className="border">
                  ...
                  <TableHead className="border">Bagian</TableHead>
              ...
              <TableBody>
                {karyawan.data.map((item, index) => (
                  <TableRow key={index} className="border">
                    ...
                    <TableHead className="border">{item.jabatan_id}</TableHead>
                    <TableHead className="border">{item.bagian}</TableHead>
            ...
            <div className="flex justify-between items-center gap-x-3 mt-5 px-10">
              ...
              <div className="flex gap-x-3">
                {karyawan.meta.links.map((link, index) => {
                  ...
                  return (
                    <Link key={index} href={link.url || ""} className={`border px-3 py-2 rounded-md aspect-square ${karyawan.meta.current_page.toString() === link.label ? "bg-stone-900 text-white" : ""}`}>
                      {link.label}
                    </Link>
                  )
                })}
